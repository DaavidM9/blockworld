...
public class Player_PreP2Test {
	...
	public void testSetFoodLevelOutOfLimit() {
		...
		if(player.getFoodLevel() != player.MAX_FOODLEVEL)
		{
			fail("Error");
		}
	}
	...
	public void testMoveError4() throws BadLocationException, EntityIsDeadException {
		...
		newpos=player.move(11111110, 3, 111111110); 
	}
	...
	//Pruebas de excepciones IllegalArgumentException 
		...
		public void testUseItemInHandExceptions2() {
			...
			try {
				player.useItemInHand(1);
				fail("No deberia haber llegado aqui");
			} catch (EntityIsDeadException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public void testSelectItem() {
		try {
			...
			ItemStack manz = new ItemStack(Material.APPLE,5);
			player.addItemsToInventory(manz);
			try {
				player.selectItem(0);
				try {
	...
	public void testToStringPlayer() {
		try {
			newpos = player.move(1, 71, 1);
			player.setHealth(10);
			player.setFoodLevel(3.95);
			//crear los items xD
